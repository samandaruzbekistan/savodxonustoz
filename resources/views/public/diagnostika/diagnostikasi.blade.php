...
<div class="flex gap-6 -mt-2">
    ...
    <div class="min-w-0 flex-1">
        ...
        {{-- Download --}}
        @php
            $file = "1.O’qish savodxonligi diagnostikasi.docx";
            $filePath = public_path("files/diagnostika/{$file}");
        @endphp
        ...
        <div class="mb-5 rounded-2xl border border-slate-200 bg-white overflow-hidden">
            ...
            <div class="px-5 py-3.5 flex items-center gap-4">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-slate-800">O'qish savodxonligi diagnostikasi</p>
                    <p class="text-xs text-slate-500 mt-0.5">DOCX &middot; {{ round(filesize($filePath)/1024) }} KB</p>
                </div>
                <a href="{{ asset('files/diagnostika/'.rawurlencode($file)) }}" download="{{ $file }}"
                   class="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 hover:border-blue-300 hover:text-blue-700 hover:bg-blue-50 transition-colors">
