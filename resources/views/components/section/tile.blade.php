...
@php
    // Static overrides: database categories that have dedicated static section pages.
    $staticRoutes = [
        'nazariya'      => 'nazariya.index',
        'metodik-modul' => 'metodik.index',
        'xalqaro-tajriba' => 'xalqaro.index',
    ];
    $categoryHref = isset($staticRoutes[$category->slug]) && \Illuminate\Support\Facades\Route::has($staticRoutes[$category->slug])
        ? route($staticRoutes[$category->slug])
        : route('sections.show', $category->slug);
...
@if ($variant === 'menu')
    <a href="{{ $categoryHref }}" class="group flex min-w-0 items-center gap-3 rounded-xl p-3 transition hover:bg-slate-50">
        <span class="grid h-9 w-9 shrink-0 place-items-center rounded-lg {{ $m['wrap'] }}">
            <x-icon :name="$m['icon']" class="h-4 w-4" />
        </span>
        <span class="min-w-0 flex-1 overflow-hidden">
            <span class="block truncate text-sm font-semibold text-slate-800 {{ $m['title'] }}">{{ $category->name }}</span>
